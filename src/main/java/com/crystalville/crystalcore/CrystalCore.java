...
public final class CrystalCore extends JavaPlugin {
    ...
    public void onEnable() {
        ...
        String payMatName = getConfig().getString(
                "pay-currency-item",
                "AMETHYST_SHARD"
        );

        String buyMatName = getConfig().getString(
                "buy-currency-item",
                "AMETHYST_SHARD"
        );
        ...
        this.payCurrency = parsedPay != null
                ? parsedPay
                : Material.AMETHYST_SHARD;

        this.buyCurrency = parsedBuy != null
                ? parsedBuy
                : Material.AMETHYST_SHARD;
        ...
        getCommand("buy").setExecutor(
                new BuyCommand(shopManager, rankManager)
        );
        getCommand("sell").setExecutor(
                new SellCommand(shopManager)
        );
        getCommand("rank").setExecutor(
                new RankCommand(this, rankManager)
        );
        getCommand("role").setExecutor(
                new RoleCommand(rankManager)
        );
        getCommand("logo").setExecutor(
                new LogoCommand()
        );
        getCommand("claim").setExecutor(
                new ClaimCommand(mailboxManager)
        );
        getCommand("i").setExecutor(
                new InspectCommand(inspectorManager)
        );
        getCommand("holefiller").setExecutor(
                new HoleFillerCommand(holeFillerManager)
        );
        getCommand("inventory").setExecutor(
                new InventoryCommand()
        );
        getCommand("shop").setExecutor(
                new ShopCommand(shopManager, rankManager)
        );
        getCommand("draft").setExecutor(
                new DraftCommand(mailboxManager)
        );
        getCommand("hud").setExecutor(
                new HudCommand(hudManager, hudListener)
        );
        getCommand("edit").setExecutor(
                new EditHudCommand(hudManager, hudListener)
        );

        // Website Player Hub linking command.
        getCommand("cvlink").setExecutor(
        ...
        getServer().getPluginManager().registerEvents(
                new HoleFillerListener(holeFillerManager),
                this
        );
        ...
        if (getConfig().getBoolean(
                "rename-amethyst-to-crystal",
                true
        )) {
            long interval = getConfig().getLong(
                    "rename-scan-interval-ticks",
                    100L
            );

            new CrystalRenameTask().runTaskTimer(
                    this,
                    40L,
                    interval
            );
        ...
        // Refresh every online player's HUD once a second.
        ...
        getLogger().info(
                "CrystalCore has been enabled. Currency: "
                        + "Crystal (Amethyst Shard) | Shop items loaded: "
                        + shopManager.getAllEntries().size()
        );
    }
