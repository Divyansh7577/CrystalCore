...
public class RankManager {
    ...
    public ChatColor resolveColor(UUID uuid) {
        String colorInput = rankColors.getOrDefault(uuid, "WHITE");
        return parseColor(colorInput);
    }

    public static ChatColor parseColor(String colorInput) {
        if (colorInput == null || colorInput.isEmpty()) {
            return ChatColor.WHITE;
        }
        try {
            return ChatColor.valueOf(colorInput.toUpperCase());
        } catch (IllegalArgumentException e) {
            String hex = colorInput.startsWith("#") ? colorInput : "#" + colorInput;
    ...
    public String getFormattedPrefix(UUID uuid) {
        if (!hasRank(uuid)) {
            return "";
        }
        ChatColor color = resolveColor(uuid);
