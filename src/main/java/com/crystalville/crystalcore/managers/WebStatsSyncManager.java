...
/**
 * Synchronizes live Minecraft player statistics with
 * the Crystal Ville website Player Hub.
 */
public final class WebStatsSyncManager {
    ...
    public void sync(Player player) {
        ...
        String apiKey = plugin.getConfig()
                .getString(
                        "website-link.supabase-publishable-key",
                        ""
                )
                .trim();
        ...
        String rank =
                ranks.hasRank(player.getUniqueId())
                        ? ranks.getRankName(player.getUniqueId())
                        : "Player";

        String endpoint =
                baseUrl
                        + "/rest/v1/rpc/sync_minecraft_profile";
                            ...
                            try {
                                ...
                                if (!success) {
                                    ...
                                    plugin.getLogger().warning(
                                            "Crystal Ville WebStats: sync failed for "
                                                    + player.getName()
                                                    + " HTTP "
                                                    + response.statusCode()
                                                    + " response="
                                ...
                                plugin.getLogger().warning(
                                        "Crystal Ville WebStats: sync error for "
                                                + player.getName()
                                                + " - "
                                                + e.getClass()
                                                        .getSimpleName()
